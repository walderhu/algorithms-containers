...
// соответсвует моему хэшу в этом узле (беру его по индексу), смотрю пустой он
// или нет, если не пустой, он должен содержать значения равные моему (я
// проверяю только первое) если одно из этих условий удовлетворяет, значит это
// нужное ведро, и мы добавляем туда наше значение, иначе мы должны расширить
// нашу структуру поскольку у нас начинают появляться коллизии (шаг расширения у
// меня в дефолте 100, я не сильно раскидываюсь в памяти) да и шанс что чел
// вкинет числа с хэшем в разницу моего шага очень мала
...
// значит мы не добавили элемент я добавляю новый узел, получаю итератор на
// него, и присваиваю ему нужное мне значение
// */
